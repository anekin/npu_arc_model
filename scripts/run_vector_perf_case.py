#!/usr/bin/env python3
"""Vector Module-Level Performance Case Runner.

End-to-end runner for a single Vector performance case.  Compiles the perf
testbench on the EDA server, runs it with plusargs, downloads the log,
and runs analyze_vector_perf.py for PASS/FAIL verdict.

Usage:
    python3 scripts/run_vector_perf_case.py --case SFV-P08 --op add --dim 128
    python3 scripts/run_vector_perf_case.py --case SFV-P08 --op add --dim 128 --commit
    python3 scripts/run_vector_perf_case.py --case SFV-P08 --op add --dim 128 --dry-run
"""
...
def step_compile_vcs(
    ...
    cmd = (
        ...
        f"-top tb_vector_perf "
        f"rtl/tb/tb_vector_perf.v rtl/vector/*.v "
...
def step_dry_run_analyze(case_id: str, op: str, dim: int) -> subprocess.CompletedProcess[str]:
    """Run analyze_vector_perf.py in dry-run mode."""
    return run_cmd(
        [
            "python3",
            str(CADUCEUS_CORE / "scripts" / "analyze_vector_perf.py"),
