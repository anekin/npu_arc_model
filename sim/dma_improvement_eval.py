...
def calc(bw_factor=1.0, weight_bits=4, sparsity=1.0):
    ...
    per_K = fill + first_cold + (N_tiles - 1) * bottleneck + drain if N_tiles >= 2 else fill + first_cold + drain
...
for name, bw, bits, sp, note in configs:
    ...
    print(
        f"{name:<30} {r['dma_per_tile']:>4}c {r['compute_per_tile']:>4}c {flag:>8} {r['gate_up_total']:>8,}c  {improvement:>+5.0f}%"
    )
    if note:
        print(f"  {'':30} {note}")
...
attn = sum(
    MXUModel(config).estimate(1, *dims).total_cycles
