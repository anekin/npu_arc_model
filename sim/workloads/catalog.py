...
class WorkloadFixture:
    ...
    def provenance_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "source_fact_refs": sorted({str(f.get("reference_uri", "")) for f in self.source_facts if f.get("reference_uri")}),
            "engineering_refs": sorted({str(a.get("reference_uri", "")) for a in self.engineering_assumptions if a.get("reference_uri")}),
        }
...
def build_qwen25_3b_graph(
    *,
    ...
    nodes.append(_make_node("embed", "gemm", inputs=["input_ids", "embed_weight"], outputs=["embed_out"], provenance=provenance))

    prev_out = "embed_out"
    for layer in range(num_layers):
        prefix = f"layer{layer}"
        t_ln1 = _make_tensor(f"{prefix}_ln1_out", ["batch", "seq_len", hidden], producer=f"{prefix}_ln1", provenance=provenance)
        tensors.append(t_ln1)
        nodes.append(_make_node(f"{prefix}_ln1", "layernorm", inputs=[prev_out], outputs=[f"{prefix}_ln1_out"], provenance=provenance))

        for proj in ("q", "k", "v"):
            t_w = _make_tensor(f"{prefix}_{proj}_weight", [hidden, hidden], producer="input", provenance=provenance)
            t_out = _make_tensor(f"{prefix}_{proj}_out", ["batch", "seq_len", hidden], producer=f"{prefix}_{proj}_proj", provenance=provenance)
            tensors.extend([t_w, t_out])
            nodes.append(_make_node(f"{prefix}_{proj}_proj", "gemm", inputs=[f"{prefix}_ln1_out", f"{prefix}_{proj}_weight"], outputs=[f"{prefix}_{proj}_out"], provenance=provenance))

        t_attn = _make_tensor(f"{prefix}_attn_out", ["batch", "seq_len", hidden], producer=f"{prefix}_attn_softmax", provenance=provenance)
        tensors.append(t_attn)
        nodes.append(_make_node(f"{prefix}_attn_softmax", "softmax", inputs=[f"{prefix}_q_out", f"{prefix}_k_out", f"{prefix}_v_out"], outputs=[f"{prefix}_attn_out"], provenance=provenance))

        t_o_w = _make_tensor(f"{prefix}_o_weight", [hidden, hidden], producer="input", provenance=provenance)
        t_o_out = _make_tensor(f"{prefix}_o_out", ["batch", "seq_len", hidden], producer=f"{prefix}_o_proj", provenance=provenance)
        tensors.extend([t_o_w, t_o_out])
        nodes.append(_make_node(f"{prefix}_o_proj", "gemm", inputs=[f"{prefix}_attn_out", f"{prefix}_o_weight"], outputs=[f"{prefix}_o_out"], provenance=provenance))

        t_add1 = _make_tensor(f"{prefix}_add1_out", ["batch", "seq_len", hidden], producer=f"{prefix}_add1", provenance=provenance)
        tensors.append(t_add1)
        nodes.append(_make_node(f"{prefix}_add1", "add", inputs=[prev_out, f"{prefix}_o_out"], outputs=[f"{prefix}_add1_out"], provenance=provenance))

        # LayerNorm 2
        t_ln2 = _make_tensor(f"{prefix}_ln2_out", ["batch", "seq_len", hidden], producer=f"{prefix}_ln2", provenance=provenance)
        tensors.append(t_ln2)
        nodes.append(_make_node(f"{prefix}_ln2", "layernorm", inputs=[f"{prefix}_add1_out"], outputs=[f"{prefix}_ln2_out"], provenance=provenance))

        # FFN expand + gate (SwiGLU style)
        for gate in ("expand", "gate"):
            t_w = _make_tensor(f"{prefix}_ffn_{gate}_weight", [hidden, intermediate], producer="input", provenance=provenance)
            t_out = _make_tensor(f"{prefix}_ffn_{gate}_out", ["batch", "seq_len", intermediate], producer=f"{prefix}_ffn_{gate}", provenance=provenance)
            tensors.extend([t_w, t_out])
            nodes.append(_make_node(f"{prefix}_ffn_{gate}", "gemm", inputs=[f"{prefix}_ln2_out", f"{prefix}_ffn_{gate}_weight"], outputs=[f"{prefix}_ffn_{gate}_out"], provenance=provenance))

        # SwiGLU mul
        t_swiglu = _make_tensor(f"{prefix}_swiglu_out", ["batch", "seq_len", intermediate], producer=f"{prefix}_swiglu", provenance=provenance)
        tensors.append(t_swiglu)
        nodes.append(_make_node(f"{prefix}_swiglu", "mul", inputs=[f"{prefix}_ffn_expand_out", f"{prefix}_ffn_gate_out"], outputs=[f"{prefix}_swiglu_out"], provenance=provenance))

        # FFN project down
        t_proj_w = _make_tensor(f"{prefix}_ffn_proj_weight", [intermediate, hidden], producer="input", provenance=provenance)
        t_proj_out = _make_tensor(f"{prefix}_ffn_proj_out", ["batch", "seq_len", hidden], producer=f"{prefix}_ffn_proj", provenance=provenance)
        tensors.extend([t_proj_w, t_proj_out])
        nodes.append(_make_node(f"{prefix}_ffn_proj", "gemm", inputs=[f"{prefix}_swiglu_out", f"{prefix}_ffn_proj_weight"], outputs=[f"{prefix}_ffn_proj_out"], provenance=provenance))

        # Residual add
        t_add2 = _make_tensor(f"{prefix}_add2_out", ["batch", "seq_len", hidden], producer=f"{prefix}_add2", provenance=provenance)
        tensors.append(t_add2)
        nodes.append(_make_node(f"{prefix}_add2", "add", inputs=[f"{prefix}_add1_out", f"{prefix}_ffn_proj_out"], outputs=[f"{prefix}_add2_out"], provenance=provenance))
    ...
    nodes.append(_make_node("final_ln", "layernorm", inputs=[prev_out], outputs=["final_ln_out"], provenance=provenance))
    ...
    nodes.append(_make_node("classifier", "gemm", inputs=["final_ln_out", "lm_head_weight"], outputs=["logits"], provenance=provenance))
...
def build_vit_b16_graph(
    *,
    ...
    t_input = _make_tensor("image", ["image_count", in_channels, image_size, image_size], producer="input", provenance=provenance)
    t_patch_w = _make_tensor("patch_embed_weight", [patch_embed_k, hidden], producer="input", provenance=provenance)
    t_patch_out = _make_tensor("patch_embed_out", ["image_count", num_patches, hidden], producer="patch_embed", provenance=provenance)
    tensors.extend([t_input, t_patch_w, t_patch_out])
    nodes.append(_make_node("patch_embed", "gemm", inputs=["image", "patch_embed_weight"], outputs=["patch_embed_out"], provenance=provenance))

    prev_out = "patch_embed_out"
    for layer in range(num_layers):
        prefix = f"block{layer}"
        t_ln1 = _make_tensor(f"{prefix}_ln1_out", ["image_count", seq_len, hidden], producer=f"{prefix}_ln1", consumers=[f"{prefix}_q_proj", f"{prefix}_k_proj", f"{prefix}_v_proj"], provenance=provenance)
        tensors.append(t_ln1)
        ...
        for proj in ("q", "k", "v"):
            t_w = _make_tensor(f"{prefix}_{proj}_weight", [hidden, hidden], producer="input", provenance=provenance)
            t_out = _make_tensor(f"{prefix}_{proj}_out", ["image_count", seq_len, hidden], producer=f"{prefix}_{proj}_proj", provenance=provenance)
            tensors.extend([t_w, t_out])
            nodes.append(_make_node(f"{prefix}_{proj}_proj", "gemm", inputs=[f"{prefix}_ln1_out", f"{prefix}_{proj}_weight"], outputs=[f"{prefix}_{proj}_out"], provenance=provenance))

        t_attn = _make_tensor(f"{prefix}_attn_out", ["image_count", num_heads, seq_len, seq_len], producer=f"{prefix}_attn_softmax", provenance=provenance)
        tensors.append(t_attn)
        nodes.append(_make_node(f"{prefix}_attn_softmax", "softmax", inputs=[f"{prefix}_q_out", f"{prefix}_k_out"], outputs=[f"{prefix}_attn_out"], provenance=provenance))

        t_v_out = next(t for t in tensors if t.tensor_id == f"{prefix}_v_out")
        t_v_out.consumed_by.append(f"{prefix}_v_mul")
        t_v_mul = _make_tensor(f"{prefix}_v_mul_out", ["image_count", seq_len, hidden], producer=f"{prefix}_v_mul", provenance=provenance)
        tensors.append(t_v_mul)
        nodes.append(_make_node(f"{prefix}_v_mul", "mul", inputs=[f"{prefix}_attn_out", f"{prefix}_v_out"], outputs=[f"{prefix}_v_mul_out"], provenance=provenance))

        t_o_w = _make_tensor(f"{prefix}_o_weight", [hidden, hidden], producer="input", provenance=provenance)
        t_o_out = _make_tensor(f"{prefix}_o_out", ["image_count", seq_len, hidden], producer=f"{prefix}_o_proj", provenance=provenance)
        tensors.extend([t_o_w, t_o_out])
        nodes.append(_make_node(f"{prefix}_o_proj", "gemm", inputs=[f"{prefix}_v_mul_out", f"{prefix}_o_weight"], outputs=[f"{prefix}_o_out"], provenance=provenance))

        t_add1 = _make_tensor(f"{prefix}_add1_out", ["image_count", seq_len, hidden], producer=f"{prefix}_add1", consumers=[f"{prefix}_ln2"], provenance=provenance)
        tensors.append(t_add1)
        prev_tensor = next(t for t in tensors if t.tensor_id == prev_out)
        ...
        t_ln2 = _make_tensor(f"{prefix}_ln2_out", ["image_count", seq_len, hidden], producer=f"{prefix}_ln2", consumers=[f"{prefix}_mlp_expand"], provenance=provenance)
        tensors.append(t_ln2)
        nodes.append(_make_node(f"{prefix}_ln2", "layernorm", inputs=[f"{prefix}_add1_out"], outputs=[f"{prefix}_ln2_out"], provenance=provenance))

        t_expand_w = _make_tensor(f"{prefix}_mlp_expand_weight", [hidden, mlp_hidden], producer="input", provenance=provenance)
        t_expand_out = _make_tensor(f"{prefix}_mlp_expand_out", ["image_count", seq_len, mlp_hidden], producer=f"{prefix}_mlp_expand", provenance=provenance)
        tensors.extend([t_expand_w, t_expand_out])
        nodes.append(_make_node(f"{prefix}_mlp_expand", "gemm", inputs=[f"{prefix}_ln2_out", f"{prefix}_mlp_expand_weight"], outputs=[f"{prefix}_mlp_expand_out"], provenance=provenance))

        t_gelu = _make_tensor(f"{prefix}_mlp_gelu_out", ["image_count", seq_len, mlp_hidden], producer=f"{prefix}_mlp_gelu", consumers=[f"{prefix}_mlp_proj"], provenance=provenance)
        tensors.append(t_gelu)
        nodes.append(_make_node(f"{prefix}_mlp_gelu", "gelu", inputs=[f"{prefix}_mlp_expand_out"], outputs=[f"{prefix}_mlp_gelu_out"], provenance=provenance))

        t_proj_w = _make_tensor(f"{prefix}_mlp_proj_weight", [mlp_hidden, hidden], producer="input", provenance=provenance)
        t_proj_out = _make_tensor(f"{prefix}_mlp_proj_out", ["image_count", seq_len, hidden], producer=f"{prefix}_mlp_proj", provenance=provenance)
        tensors.extend([t_proj_w, t_proj_out])
        nodes.append(_make_node(f"{prefix}_mlp_proj", "gemm", inputs=[f"{prefix}_mlp_gelu_out", f"{prefix}_mlp_proj_weight"], outputs=[f"{prefix}_mlp_proj_out"], provenance=provenance))

        t_add2 = _make_tensor(f"{prefix}_add2_out", ["image_count", seq_len, hidden], producer=f"{prefix}_add2", provenance=provenance)
        tensors.append(t_add2)
        nodes.append(_make_node(f"{prefix}_add2", "add", inputs=[f"{prefix}_add1_out", f"{prefix}_mlp_proj_out"], outputs=[f"{prefix}_add2_out"], provenance=provenance))

        prev_out = f"{prefix}_add2_out"

    t_final_ln = _make_tensor("final_ln_out", ["image_count", seq_len, hidden], producer="final_ln", provenance=provenance)
    tensors.append(t_final_ln)
    nodes.append(_make_node("final_ln", "layernorm", inputs=[prev_out], outputs=["final_ln_out"], provenance=provenance))
    ...
    nodes.append(_make_node("classifier", "gemm", inputs=["final_ln_out", "classifier_weight"], outputs=["logits"], provenance=provenance))
...
def build_yolov8n_graph(
    *,
    ...
    def add_conv(name: str, in_shape: list[int | str], weight_shape: list[int], out_shape: list[int | str]) -> str:
        ...
        if t_in is None:
            t_in = _make_tensor(str(in_shape[0]), in_shape, producer="input" if in_shape[0] == "image" else "", provenance=provenance)
            tensors.append(t_in)
        ...
        tensors.extend([t_w, t_out])
        nodes.append(_make_node(name, "conv", inputs=[str(in_shape[0]), f"{name}_weight"], outputs=[out_id], provenance=provenance))
        return out_id
    ...
    nodes.append(_make_node("p5_upsample", "reshape", inputs=[sppf], outputs=["p5_upsample_out"], provenance=provenance))
    ...
    for scale, channels, h_size, feat in (("p3", 64, 80, p3), ("p4", 128, 40, p4_2), ("p5", 256, 20, p5_2)):
        b0 = add_conv(f"head_{scale}_box0", [feat], [channels, channels, 3, 3], ["image_count", channels, h_size, h_size])
...
def build_smolvla_graph(
    *,
    ...
    patch_out = _make_tensor("vit_patch_out", ["image_count", 197, vision_hidden], producer="vit_patch_embed", provenance=provenance)
    tensors.extend([patch_w, patch_out])
    nodes.append(_make_node("vit_patch_embed", "gemm", inputs=["image", "vit_patch_weight"], outputs=["vit_patch_out"], provenance=provenance))

    prev = "vit_patch_out"
    for layer in range(vision_layers):
        prefix = f"vit{layer}"
        ln = _make_tensor(f"{prefix}_ln_out", ["image_count", 197, vision_hidden], producer=f"{prefix}_ln", provenance=provenance)
        tensors.append(ln)
        nodes.append(_make_node(f"{prefix}_ln", "layernorm", inputs=[prev], outputs=[f"{prefix}_ln_out"], provenance=provenance))
        q_w = _make_tensor(f"{prefix}_q_weight", [vision_hidden, vision_hidden], producer="input", provenance=provenance)
        q_out = _make_tensor(f"{prefix}_q_out", ["image_count", 197, vision_hidden], producer=f"{prefix}_q_proj", provenance=provenance)
        k_w = _make_tensor(f"{prefix}_k_weight", [vision_hidden, vision_hidden], producer="input", provenance=provenance)
        k_out = _make_tensor(f"{prefix}_k_out", ["image_count", 197, vision_hidden], producer=f"{prefix}_k_proj", provenance=provenance)
        v_w = _make_tensor(f"{prefix}_v_weight", [vision_hidden, vision_hidden], producer="input", provenance=provenance)
        v_out = _make_tensor(f"{prefix}_v_out", ["image_count", 197, vision_hidden], producer=f"{prefix}_v_proj", provenance=provenance)
        o_w = _make_tensor(f"{prefix}_o_weight", [vision_hidden, vision_hidden], producer="input", provenance=provenance)
        o_out = _make_tensor(f"{prefix}_o_out", ["image_count", 197, vision_hidden], producer=f"{prefix}_o_proj", provenance=provenance)
        tensors.extend([q_w, q_out, k_w, k_out, v_w, v_out, o_w, o_out])
        nodes.extend([
            _make_node(f"{prefix}_q_proj", "gemm", inputs=[f"{prefix}_ln_out", f"{prefix}_q_weight"], outputs=[f"{prefix}_q_out"], provenance=provenance),
            _make_node(f"{prefix}_k_proj", "gemm", inputs=[f"{prefix}_ln_out", f"{prefix}_k_weight"], outputs=[f"{prefix}_k_out"], provenance=provenance),
            _make_node(f"{prefix}_v_proj", "gemm", inputs=[f"{prefix}_ln_out", f"{prefix}_v_weight"], outputs=[f"{prefix}_v_out"], provenance=provenance),
            _make_node(f"{prefix}_o_proj", "gemm", inputs=[f"{prefix}_v_out", f"{prefix}_o_weight"], outputs=[f"{prefix}_o_out"], provenance=provenance),
        ])
    ...
    proj_out = _make_tensor("vision_proj_out", ["image_count", 197, vl_hidden], producer="vision_proj", provenance=provenance)
    tensors.extend([proj_w, proj_out])
    nodes.append(_make_node("vision_proj", "gemm", inputs=[prev, "vision_proj_weight"], outputs=["vision_proj_out"], provenance=provenance))

    vl_ln = _make_tensor("vl_ln_out", ["image_count", 197, vl_hidden], producer="vl_ln", provenance=provenance)
    tensors.append(vl_ln)
    nodes.append(_make_node("vl_ln", "layernorm", inputs=["vision_proj_out"], outputs=["vl_ln_out"], provenance=provenance))
    for vl_layer in range(vl_layers):
        prefix = f"vl{vl_layer}"
        ffn_w = _make_tensor(f"{prefix}_ffn_weight", [vl_hidden, vl_hidden * 4], producer="input", provenance=provenance)
        ffn_out = _make_tensor(f"{prefix}_ffn_out", ["image_count", 197, vl_hidden * 4], producer=f"{prefix}_ffn", provenance=provenance)
        gelu_out = _make_tensor(f"{prefix}_gelu_out", ["image_count", 197, vl_hidden * 4], producer=f"{prefix}_gelu", provenance=provenance)
        proj2_w = _make_tensor(f"{prefix}_proj_weight", [vl_hidden * 4, vl_hidden], producer="input", provenance=provenance)
        proj2_out = _make_tensor(f"{prefix}_proj_out", ["image_count", 197, vl_hidden], producer=f"{prefix}_proj", provenance=provenance)
        tensors.extend([ffn_w, ffn_out, gelu_out, proj2_w, proj2_out])
        nodes.extend([
            _make_node(f"{prefix}_ffn", "gemm", inputs=["vl_ln_out" if vl_layer == 0 else f"vl{vl_layer - 1}_proj_out", f"{prefix}_ffn_weight"], outputs=[f"{prefix}_ffn_out"], provenance=provenance),
            _make_node(f"{prefix}_gelu", "gelu", inputs=[f"{prefix}_ffn_out"], outputs=[f"{prefix}_gelu_out"], provenance=provenance),
            _make_node(f"{prefix}_proj", "gemm", inputs=[f"{prefix}_gelu_out", f"{prefix}_proj_weight"], outputs=[f"{prefix}_proj_out"], provenance=provenance),
        ])
    ...
    for step in range(flow_steps):
        ...
        out = _make_tensor(f"{prefix}_out", ["batch", action_dim * action_horizon], producer=f"{prefix}_fc2", provenance=provenance)
        tensors.extend([w1, h, gelu_out, w2, out])
        nodes.extend([
            _make_node(f"{prefix}_fc1", "gemm", inputs=["action_in" if step == 0 else f"flow{step - 1}_out", f"{prefix}_w1"], outputs=[f"{prefix}_hidden"], provenance=provenance),
            _make_node(f"{prefix}_gelu", "gelu", inputs=[f"{prefix}_hidden"], outputs=[f"{prefix}_gelu_out"], provenance=provenance),
            _make_node(f"{prefix}_fc2", "gemm", inputs=[f"{prefix}_gelu_out", f"{prefix}_w2"], outputs=[f"{prefix}_out"], provenance=provenance),
        ])
...
def build_pi0_graph(
    *,
    ...
    patch_w = _make_tensor("vit_patch_weight", [588, vision_hidden], producer="input", provenance=provenance)
    patch_out = _make_tensor("vit_patch_out", ["batch", 197, vision_hidden], producer="vit_patch_embed", provenance=provenance)
    tensors.extend([patch_w, patch_out])
    nodes.append(_make_node("vit_patch_embed", "gemm", inputs=["images", "vit_patch_weight"], outputs=["vit_patch_out"], provenance=provenance))

    ln_out = _make_tensor("vit_ln_out", ["batch", 197, vision_hidden], producer="vit_ln", provenance=provenance)
    tensors.append(ln_out)
    nodes.append(_make_node("vit_ln", "layernorm", inputs=["vit_patch_out"], outputs=["vit_ln_out"], provenance=provenance))

    proj_w = _make_tensor("vision_proj_weight", [vision_hidden, vl_hidden], producer="input", provenance=provenance)
    proj_out = _make_tensor("vision_proj_out", ["batch", 197, vl_hidden], producer="vision_proj", provenance=provenance)
    tensors.extend([proj_w, proj_out])
    nodes.append(_make_node("vision_proj", "gemm", inputs=["vit_ln_out", "vision_proj_weight"], outputs=["vision_proj_out"], provenance=provenance))
    ...
    for vl_layer in range(4):
        prefix = f"vl{vl_layer}"
        ffn_w = _make_tensor(f"{prefix}_ffn_weight", [vl_hidden, vl_hidden * 4], producer="input", provenance=provenance)
        ffn_out = _make_tensor(f"{prefix}_ffn_out", ["batch", 197, vl_hidden * 4], producer=f"{prefix}_ffn", provenance=provenance)
        gelu_out = _make_tensor(f"{prefix}_gelu_out", ["batch", 197, vl_hidden * 4], producer=f"{prefix}_gelu", provenance=provenance)
        proj2_w = _make_tensor(f"{prefix}_proj_weight", [vl_hidden * 4, vl_hidden], producer="input", provenance=provenance)
        proj2_out = _make_tensor(f"{prefix}_proj_out", ["batch", 197, vl_hidden], producer=f"{prefix}_proj", provenance=provenance)
        tensors.extend([ffn_w, ffn_out, gelu_out, proj2_w, proj2_out])
        nodes.extend([
            _make_node(f"{prefix}_ffn", "gemm", inputs=["vl_ln_out" if vl_layer == 0 else f"vl{vl_layer - 1}_proj_out", f"{prefix}_ffn_weight"], outputs=[f"{prefix}_ffn_out"], provenance=provenance),
            _make_node(f"{prefix}_gelu", "gelu", inputs=[f"{prefix}_ffn_out"], outputs=[f"{prefix}_gelu_out"], provenance=provenance),
    ...
    nodes.append(_make_node("visual_pool", "global_avg_pool", inputs=["vl3_proj_out"], outputs=["visual_pool_out"], provenance=provenance))

    t_proprio = _make_tensor("proprioception", ["batch", action_dim], producer="input", provenance=provenance)
    tensors.append(t_proprio)
    fuse_concat = _make_tensor("fuse_concat_out", ["batch", vl_hidden + action_dim], producer="fuse_concat", provenance=provenance)
    tensors.append(fuse_concat)
    nodes.append(_make_node("fuse_concat", "concat", inputs=["visual_pool_out", "proprioception"], outputs=["fuse_concat_out"], provenance=provenance))
    ...
    nodes.append(_make_node("fusion", "gemm", inputs=["fuse_concat_out", "fusion_weight"], outputs=["fusion_out"], provenance=provenance))
    ...
    for step in range(flow_steps):
        ...
        tensors.extend([w1, h, gelu_out, w2, out])
        nodes.extend([
            _make_node(f"{prefix}_fc1", "gemm", inputs=["fusion_out" if step == 0 else f"flow{step - 1}_out", f"{prefix}_w1"], outputs=[f"{prefix}_hidden"], provenance=provenance),
            _make_node(f"{prefix}_gelu", "gelu", inputs=[f"{prefix}_hidden"], outputs=[f"{prefix}_gelu_out"], provenance=provenance),
            _make_node(f"{prefix}_fc2", "gemm", inputs=[f"{prefix}_gelu_out", f"{prefix}_w2"], outputs=[f"{prefix}_out"], provenance=provenance),
        ])
...
def build_openvla_graph(
    *,
    ...
    proj_out = _make_tensor("vision_proj_out", ["image_count", 197, vl_hidden], producer="vision_proj", provenance=provenance)
    tensors.extend([proj_w, proj_out])
    nodes.append(_make_node("vision_proj", "gemm", inputs=["vit_patch_out", "vision_proj_weight"], outputs=["vision_proj_out"], provenance=provenance))

    prev = "vision_proj_out"
    for vl_layer in range(2):
        prefix = f"vl{vl_layer}"
        ln = _make_tensor(f"{prefix}_ln_out", ["image_count", 197, vl_hidden], producer=f"{prefix}_ln", provenance=provenance)
        tensors.append(ln)
        nodes.append(_make_node(f"{prefix}_ln", "layernorm", inputs=[prev], outputs=[f"{prefix}_ln_out"], provenance=provenance))
        ...
        tensors.extend([ffn_w, ffn_out, gelu_out, proj2_w, proj2_out])
        nodes.extend([
            _make_node(f"{prefix}_ffn", "gemm", inputs=[f"{prefix}_ln_out", f"{prefix}_ffn_weight"], outputs=[f"{prefix}_ffn_out"], provenance=provenance),
            _make_node(f"{prefix}_gelu", "gelu", inputs=[f"{prefix}_ffn_out"], outputs=[f"{prefix}_gelu_out"], provenance=provenance),
            _make_node(f"{prefix}_proj", "gemm", inputs=[f"{prefix}_gelu_out", f"{prefix}_proj_weight"], outputs=[f"{prefix}_proj_out"], provenance=provenance),
        ])
        prev = f"{prefix}_proj_out"
    ...
    pool_out = _make_tensor("visual_pool_out", ["batch", vl_hidden], producer="visual_pool", provenance=provenance)
    tensors.append(pool_out)
    nodes.append(_make_node("visual_pool", "global_avg_pool", inputs=[prev], outputs=["visual_pool_out"], provenance=provenance))
    ...
    if variant == "fast":
        ...
        compress_out = _make_tensor("token_compress_out", ["image_count", 197, 256], producer="token_compress", provenance=provenance)
        tensors.extend([compress_w, compress_out])
        nodes.append(_make_node("token_compress", "gemm", inputs=[prev, "token_compress_weight"], outputs=["token_compress_out"], provenance=provenance))
        pool_out_fast = _make_tensor("fast_pool_out", ["batch", 256], producer="fast_pool", provenance=provenance)
        tensors.append(pool_out_fast)
        nodes.append(_make_node("fast_pool", "global_avg_pool", inputs=["token_compress_out"], outputs=["fast_pool_out"], provenance=provenance))
    ...
    for step in range(flow_steps):
        ...
        gelu_out = _make_tensor(f"{prefix}_gelu_out", ["batch", head_hidden], producer=f"{prefix}_gelu", provenance=provenance)
        w2 = _make_tensor(f"{prefix}_w2", [head_hidden, action_dim * action_horizon], producer="input", provenance=provenance)
        out = _make_tensor(f"{prefix}_out", ["batch", action_dim * action_horizon], producer=f"{prefix}_fc2", provenance=provenance)
        tensors.extend([w1, h, gelu_out, w2, out])
        nodes.extend([
            _make_node(f"{prefix}_fc1", "gemm", inputs=[action_input if step == 0 else f"flow{step - 1}_out", f"{prefix}_w1"], outputs=[f"{prefix}_hidden"], provenance=provenance),
            _make_node(f"{prefix}_gelu", "gelu", inputs=[f"{prefix}_hidden"], outputs=[f"{prefix}_gelu_out"], provenance=provenance),
            _make_node(f"{prefix}_fc2", "gemm", inputs=[f"{prefix}_gelu_out", f"{prefix}_w2"], outputs=[f"{prefix}_out"], provenance=provenance),
        ])
...
def build_helix_graph(
    *,
    ...
    s2_patch_out = _make_tensor("s2_patch_out", ["image_count", 197, s2_hidden], producer="s2_patch_embed", provenance=provenance)
    tensors.extend([s2_patch_w, s2_patch_out])
    nodes.append(_make_node("s2_patch_embed", "gemm", inputs=["image", "s2_patch_weight"], outputs=["s2_patch_out"], provenance=provenance))
    ...
    nodes.append(_make_node("s2_ln", "layernorm", inputs=["s2_patch_out"], outputs=["s2_ln_out"], provenance=provenance))

    s2_ffn_w = _make_tensor("s2_ffn_weight", [s2_hidden, s2_hidden * 4], producer="input", provenance=provenance)
    s2_ffn_out = _make_tensor("s2_ffn_out", ["image_count", 197, s2_hidden * 4], producer="s2_ffn", provenance=provenance)
    s2_gelu_out = _make_tensor("s2_gelu_out", ["image_count", 197, s2_hidden * 4], producer="s2_gelu", provenance=provenance)
    s2_proj_w = _make_tensor("s2_proj_weight", [s2_hidden * 4, s2_hidden], producer="input", provenance=provenance)
    s2_proj_out = _make_tensor("s2_proj_out", ["image_count", 197, s2_hidden], producer="s2_proj", provenance=provenance)
    tensors.extend([s2_ffn_w, s2_ffn_out, s2_gelu_out, s2_proj_w, s2_proj_out])
    nodes.extend([
        _make_node("s2_ffn", "gemm", inputs=["s2_ln_out", "s2_ffn_weight"], outputs=["s2_ffn_out"], provenance=provenance),
        _make_node("s2_gelu", "gelu", inputs=["s2_ffn_out"], outputs=["s2_gelu_out"], provenance=provenance),
        _make_node("s2_proj", "gemm", inputs=["s2_gelu_out", "s2_proj_weight"], outputs=["s2_proj_out"], provenance=provenance),
    ])

    s2_pool = _make_tensor("s2_latent", ["batch", s2_hidden], producer="s2_pool", provenance=provenance)
    tensors.append(s2_pool)
    nodes.append(_make_node("s2_pool", "global_avg_pool", inputs=["s2_proj_out"], outputs=["s2_latent"], provenance=provenance))
    ...
    nodes.extend([
        ...
        _make_node("s1_fc1", "gemm", inputs=["s1_fused", "s1_w1"], outputs=["s1_hidden_out"], provenance=provenance),
    ...
    action_concat = _make_tensor("action_concat_out", ["batch", s1_hidden + s0_hidden], producer="action_concat", provenance=provenance)
    tensors.append(action_concat)
    nodes.append(_make_node("action_concat", "concat", inputs=["s1_out", "s0_out"], outputs=["action_concat_out"], provenance=provenance))
    ...
    nodes.append(_make_node("action_head", "gemm", inputs=["action_concat_out", "action_w"], outputs=["action_out"], provenance=provenance))
...
def build_physical_ai_multijob_graph(
    *,
    ...
    for job_name, batch_axis, channels, h, w, out_channels in jobs:
        ...
        t_w = _make_tensor(f"{job_name}_weight", [out_channels, channels, 3, 3], producer="input", provenance=provenance)
        t_out = _make_tensor(f"{job_name}_feature", [batch_axis, out_channels, h, w], producer=f"{job_name}_conv", provenance=provenance)
        tensors.extend([t_in, t_w, t_out])
        nodes.append(_make_node(f"{job_name}_conv", "conv", inputs=[f"{job_name}_input", f"{job_name}_weight"], outputs=[f"{job_name}_feature"], provenance=provenance))

        t_pool = _make_tensor(f"{job_name}_pool", [batch_axis, out_channels], producer=f"{job_name}_pool", provenance=provenance)
        tensors.append(t_pool)
        nodes.append(_make_node(f"{job_name}_pool", "global_avg_pool", inputs=[f"{job_name}_feature"], outputs=[f"{job_name}_pool"], provenance=provenance))
