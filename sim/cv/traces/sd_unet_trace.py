...
def _conv_im2col(name: str, M: int, K: int, N: int, overhead: float = 0) -> dict[str, Any]:
...
def _resnet_block(prefix: str, tokens: int, in_ch: int, out_ch: int, time_emb_dim: int = 1280) -> list[dict[str, Any]]:
...
def _self_attn_block(prefix: str, tokens: int, channels: int, num_heads: int) -> list[dict[str, Any]]:
    ...
    # Attention softmax
    ops.append(_sfu(f"{prefix}_softmax", num_heads * tokens * tokens, "softmax"))
    # Output projection
...
def _cross_attn_block(
    prefix: str, latent_tokens: int, text_tokens: int, channels: int, cross_dim: int, num_heads: int
) -> list[dict[str, Any]]:
    ...
    ops.append(_sfu(f"{prefix}_softmax", num_heads * latent_tokens * text_tokens, "softmax"))
    # Output projection
...
def generate_sd_unet_step() -> list[dict[str, Any]]:
    ...
    for level in range(4):
        ...
        for b in range(_RESNET_BLOCKS[level]):
            ...
            trace.extend(_resnet_block(f"down_l{level}_b{b}", spatial_tokens, ch_in, out_ch))

        # Self-attention + cross-attention at specific levels
        if level in _ATTN_RESOLUTIONS:
            trace.extend(_self_attn_block(f"down_l{level}_selfattn", spatial_tokens, out_ch, _NUM_HEADS[level]))
            trace.extend(
                _cross_attn_block(
                    f"down_l{level}_crossattn", spatial_tokens, _TEXT_TOKENS, out_ch, _CROSS_ATTN_DIM, _NUM_HEADS[level]
                )
            )
        ...
        if level < 3:
            trace.append(_sfu(f"down_l{level}_downsample", spatial_tokens * out_ch, "silu"))
            new_tokens = spatial_tokens // 4
            k = 3 * 3 * out_ch
            trace.append(_conv_im2col(f"down_l{level}_down_conv", new_tokens, k, out_ch))
            spatial_tokens = new_tokens

    # ── Middle block ──
    mid_ch = _CHANNELS[-1]  # 1280
    trace.extend(_resnet_block("mid_b0", spatial_tokens, mid_ch, mid_ch))
    trace.extend(_self_attn_block("mid_selfattn", spatial_tokens, mid_ch, _NUM_HEADS[-1]))
    trace.extend(
        _cross_attn_block("mid_crossattn", spatial_tokens, _TEXT_TOKENS, mid_ch, _CROSS_ATTN_DIM, _NUM_HEADS[-1])
    )
    ...
    for level in range(3, -1, -1):
        ...
        if level < 3:
            new_tokens = spatial_tokens * 4
            trace.append(_sfu(f"up_l{level}_upsample", spatial_tokens * in_ch, "silu"))
            k = 3 * 3 * in_ch
            trace.append(_conv_im2col(f"up_l{level}_up_conv", new_tokens, k, in_ch))
            spatial_tokens = new_tokens
        ...
        for b in range(_RESNET_BLOCKS[level] + 1):
            ...
            trace.extend(_resnet_block(f"up_l{level}_b{b}", spatial_tokens, ch_in, out_ch))

        # Attention at specific levels
        if level in _ATTN_RESOLUTIONS:
            trace.extend(_self_attn_block(f"up_l{level}_selfattn", spatial_tokens, out_ch, _NUM_HEADS[level]))
            trace.extend(
                _cross_attn_block(
                    f"up_l{level}_crossattn", spatial_tokens, _TEXT_TOKENS, out_ch, _CROSS_ATTN_DIM, _NUM_HEADS[level]
                )
            )
...
if __name__ == "__main__":
    ...
    print(f"  GEMM: {len(gemm_entries) - len(conv_entries)}, Conv: {len(conv_entries)}, SFU: {len(sfu_entries)}")
    print(f"  MACs: {total_macs:,} ({total_macs / 1e9:.1f} G) ≈ {total_macs * 2 / 1e12:.1f} TFLOPs")
