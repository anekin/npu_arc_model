...
class SWOverheadModel:
    ...
    def estimate(
        self,
        num_layers: int,
        num_tiles_per_token: int,
        num_isa_instructions: int,
        has_dma_chain: bool = True,
        kv_evict_per_layer: bool = False,
        dequant_on_chip: bool = False,
    ) -> SWOverheadResult:
    ...
    def estimate_for_engine(
        self,
        engine_name: str,
        num_layers: int = 28,
        num_isa_per_layer: int = 10,  # avg ISA insts per layer
        tiles_per_layer: dict[str, int] = None,
    ) -> SWOverheadResult:
        """按引擎预估 (含 tile 数差异)"""
