...
def check_model_consistency() -> list[str]:
    ...
    for pattern, fix in broken_import_patterns:
        result = _subprocess.run(
            ["grep", "-rn", pattern, "--include=*.py", str(SIM_DIR)], capture_output=True, text=True
        )
        ...
        if sweep_path.exists():
            ...
            for val in target_matches:
                if int(val) != TARGET_TOK_S:
                    issues.append(
                        f"{sweep_file}: hardcoded target={val} (should be {TARGET_TOK_S} from overnight_loop.py)"
                    )
...
def run_e2e() -> dict[str, Any]:
    ...
    try:
        ...
        for line in all_lines:
            ...
            if "单 token" in line and "tok/s" in line and str(TARGET_TOK_S) in line and "✅" in line:
                target_met = True
            # Batch M=2 check
    ...
    for issue in issues:
        ...
        if "weight_preloaded" in issue:
            ...
            with open(sim_path, "w") as f:
                f.write(content)
            fixed += 1
            log("    Fixed weight_preloaded in npu_sim.py")
            ...
            with open(compiler_path, "w") as f:
                f.write(content)
            fixed += 1
            log("    Fixed weight_preloaded default in compiler.py")
            ...
            if m:
                ...
                log("    Auto-fix not available for broken import — requires manual review")
...
def generate_summary(iter_n: int, issues: list[str], sweep: dict, e2e: dict):
        ...
        for r in sweep["all_results"]:
            if "M=1" in r.get("config", ""):
                ...
                if tok > 0:
                    ...
                    break
    dram_available = _cfg.get("memory", {}).get("bandwidth_gbps", 51.2) * _cfg.get("memory", {}).get(
    ...
    health.append(
        f"| weight_preloaded removed | {'✅' if wp_ok else '❌'} | {'Clean' if wp_ok else 'Residual found'} |"
    )
    ...
    health.append(
        f"| dram_efficiency: 0.85 | {'✅' if de_ok else '❌'} | {'85% effective BW' if de_ok else 'Missing/wrong'} |"
    )
    ...
    health.append(
        f"| MXU v2 tiling model | {'✅' if v2_ok else '❌'} | {'tile_weight_bytes + dram_efficiency' if v2_ok else 'Missing v2 markers'} |"
    )
    ...
    health.append(
        f"| validate_e2e uses v2 MXU | {'✅' if e2e_v2 else '❌'} | {'Imports MXUModel from models.mxu' if e2e_v2 else 'Wrong import'} |"
    )
    ...
    health.append(
        f"| DRAM BW (demand vs effective) | {'✅' if bw_ok else '⚠️'} | {dram_demand:.1f} / {dram_available} GB/s ({bw_pct:.0f}%) → {bottleneck} |"
    )
    ...
    for eng in engine_dir.glob("*.py"):
        ...
        if "weight_preloaded: bool = True" in ec:
            ...
            break
    health.append(
        f"| Engine weight_preloaded=False | {'✅' if engines_ok else '❌'} | {'All engines v2-compliant' if engines_ok else 'Found True default'} |"
    )
...
def check_self() -> list[str]:
    ...
    for i, line in enumerate(source.split("\n"), 1):
        ...
        if ('f"' in stripped or "f'" in stripped) and ("tok/s" in stripped or "GB/s" in stripped or "GB" in stripped):
    ...
    for i, line in enumerate(source.split("\n"), 1):
        ...
        if re.search(r"[><=]=\s*(?:25|31)\b", stripped) and ("TARGET_TOK_S" not in stripped and "25," not in stripped):
