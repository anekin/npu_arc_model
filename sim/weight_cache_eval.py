...
def estimate_weight_cache(M, K, N):
    ...
    per_Ktile = fill + first_cold + (N_tiles - 1) * bottleneck + drain if N_tiles >= 2 else fill + first_cold + drain
...
# ── HW cost ──
print("\n  Hardware cost:")
print("    PE weight reg: 4-bit → 8-bit (+4 bits per PE)")
print("    PE area:       ~+15% (1 extra FF + MUX per PE)")
print("    Array area:    8mm² → ~9.2mm²")
print("    Total chip:    27mm² → ~28mm²")

# ── Combined with batching ──
print("\n  Combined with batching (M=2):")
# Quick batch estimate
batch_current = sum(
    mxu.estimate(2, K, N).total_cycles
    for _, K, N in [
        ("Q", HIDDEN, 4096),
        ("K", HIDDEN, 256),
        ("V", HIDDEN, 256),
...
attn_m2 = (
    mxu.estimate(2, HIDDEN, 4096).total_cycles
