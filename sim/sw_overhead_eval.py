...
def main():
    ...
    with open(Path(__file__).parent / "config" / "npu_config.yaml") as _config_file:
    ...
    # 表头
    print(
        f"{'Engine':20s} {'HW tok/s':>7s} {'HW μs':>8s}  {'SW(A)%':>6s} {'SW(B)%':>6s} {'SW(C)%':>6s}  A tok/s  B tok/s  C tok/s"
    )
    print("-" * 100)
    ...
    for eng_name in engines:
        ...
        print(
            f"{eng_name:20s} {hw_tok:7.0f} {hw_us:8.0f}  {pct_a:5.1f}% {pct_b:5.1f}% {pct_c:5.1f}%  {tok_a:7.0f} {tok_b:7.0f} {tok_c:7.0f}"
        )
    ...
    print()
    print("  关键发现:")
    print("    1. DMA descriptor chain 至关重要 — 无 chain 时 per-tile 开销累计")
    print("    2. Host PCIe 开销 (100μs) 对小 batch 影响显著")
    print("    3. 快引擎 (block/gmma) 受 SW 开销影响最大 (HW 时间短, SW 占比高)")
    print("    4. 慢引擎 (systolic/wmma) SW 占比可忽略")
