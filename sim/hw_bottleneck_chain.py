...
print("  瓶颈: pipeline fill/drain — 385c overhead vs 128 MAC useful work")
...
print(
    f"  Gate+Up: {585391 * 2 / 1000:.0f}μs → {gate_up_cached / 1000:.0f}μs (节省 {585391 * 2 - gate_up_cached:.0f} cycles)"
)
...
print(
    f"  Gate+Up: {gate_up_cached / 1000:.0f}μs → {gate_up_l2 / 1000:.0f}μs (节省 {gate_up_cached - gate_up_l2:.0f} cycles)"
)
...
for h_test in [128, 96, 64]:
    ...
    tok = 1e6 / (
        (
            ...
            + down.total_cycles
            + sum(
                mxu.estimate(1, k, n).total_cycles
                for k, n in [(HIDDEN, 4096), (HIDDEN, 256), (HIDDEN, 256), (4096, HIDDEN)]
            )
        )
        * NUM_LAYERS
        / 1000
    ...
    print(
        f"    H={h_test}: tiles={kt}×{nt}={n_tiles_total}, drain={drain_test}c, "
...
print(
    f"  Gate+Up theoretical minimum: {ideal_total / 1000:.0f}μs "
...
print("  瓶颈            来源              改进方案          剩余")
print("  ─────────────────────────────────────────────────────")
print("  1. pipeline fill/drain  tile开销    权重缓存(PE双reg)  → DMA等待")
print("  2. DMA 等待          双倍权重加载   +128bit/DMA通道    → drain等待")
print("  3. pipeline drain    M+H=129c/tile  物理极限(结构性的)  → 无法硬件消除")
print("")
print("  结论: 纯硬件路径走到头是 ~24 tok/s。剩下的 drain 是 systolic")
print("  array 在 M=1 下的物理极限 — 信号必须走 128 列才能出结果。")
print("  突破需要: output-stationary 数据流 或 不同 microarchitecture。")
